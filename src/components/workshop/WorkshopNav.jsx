...
const NAV_ITEMS = [
  ...
  { id: 'find-jugaad', label: 'FIND A JUGAAD', icon: Search, path: '/dashboard/find-jugaad' },
  { id: 'post-jugaad', label: 'POST A JUGAAD', icon: Plus, path: '/dashboard/post-jugaad' },
  { id: 'my-jugaads', label: 'MY JUGAADS', icon: ClipboardList, path: '/dashboard/my-jugaads' },
  { id: 'requests', label: 'REQUESTS', icon: Inbox, path: '/dashboard/requests' },
  { id: 'my-requests', label: 'MY REQUESTS', icon: Send, path: '/dashboard/my-requests' },
...
export function WorkshopNav() {
  ...
  const getActiveId = () => {
    ...
    if (path.includes('find-jugaad')) return 'find-jugaad';
    if (path.includes('post-jugaad')) return 'post-jugaad';
    if (path.includes('my-jugaads')) return 'my-jugaads';
    ...
    <>
      <header className="sticky top-0 z-50 px-4 pt-4">
        <nav className="surface-metal-brushed metal-scratches relative max-w-7xl mx-auto rounded-2xl px-4 sm:px-5 py-2.5 flex items-center justify-between gap-2">
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  key={item.id}
                  to={item.path}
                  className={`group relative flex items-center gap-1.5 px-2.5 py-2 rounded-lg transition-colors ${isActive ? 'text-amber-soft' : 'text-ink-1 hover:text-ink-0'}`}
                >
          ...
          {/* Right — notifications, messages, user, logout */}
          <div className="hidden lg:flex items-center gap-1.5 shrink-0">
            ...
            <Link
              ...
              style={{ background: activeId === 'messages' ? 'rgba(93,184,154,0.08)' : 'rgba(255,255,255,0.03)' }}
              aria-label="Messages"
            >
            ...
            <Link
              ...
              style={{ background: activeId === 'profile' ? 'rgba(214,138,60,0.08)' : 'rgba(255,255,255,0.03)' }}
            >
              ...
              <span className="font-mono text-[9px] text-ink-0 max-w-[80px] truncate">{user?.name || 'Operator'}</span>
              {isDemoMode && (
                <span className="font-technical text-[6px] text-amber px-1.5 py-0.5 rounded" style={{ border: '1px solid rgba(214,138,60,0.4)', background: 'rgba(214,138,60,0.1)' }}>
                  DEMO
                </span>
              )}
            </Link>
        ...
        {mobileOpen && (
          <div className="lg:hidden absolute top-full left-4 right-4 mt-2 surface-panel rounded-2xl p-4 flex flex-col gap-1 anim-reveal z-50">
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg transition-colors ${isActive ? 'text-amber-soft bg-white/5' : 'text-ink-0 hover:text-amber-soft'}`}
                >
            ...
            <Link
              to="/dashboard/messages"
              onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg transition-colors ${activeId === 'messages' ? 'text-mint-soft bg-white/5' : 'text-ink-0 hover:text-mint-soft'}`}
            >
              <MessageSquare size={14} />
              <span className="font-technical text-[10px]">MESSAGES</span>
            </Link>
            <Link
              to="/profile"
              onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg transition-colors ${activeId === 'profile' ? 'text-amber-soft bg-white/5' : 'text-ink-0 hover:text-amber-soft'}`}
            >
              <User size={14} />
              <span className="font-technical text-[10px]">PROFILE</span>
            </Link>
            <Link
              to="/settings"
              onClick={() => setMobileOpen(false)}
              className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg transition-colors ${activeId === 'settings' ? 'text-amber-soft bg-white/5' : 'text-ink-0 hover:text-amber-soft'}`}
            >
              <Settings size={14} />
              <span className="font-technical text-[10px]">SETTINGS</span>
            </Link>
            <div className="h-px bg-metal-1 my-1" />
            <button
              onClick={handleLogout}
