...
export function FindJugaadPage() {
  ...
  const fetchFeed = useCallback(async () => {
    ...
    try {
      ...
      setFeedItems(Array.isArray(list) ? list.filter(Boolean) : []);
    } catch (error) {
      console.error('Failed to load Jugaad feed:', error);
      setFeedItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const sections = [
    [
      'RECOMMENDED FOR YOU',
      visible.filter((x) => x.matchPercentage >= 80),
    ],
    [
      'BEST SKILL MATCHES',
      visible.filter(
        (x) => x.matchPercentage >= 70 && x.matchPercentage < 80
      ),
    ],
    [
      'RECENTLY POSTED',
      visible
        .filter((x) => x.postedAt >= '2026-08-21')
        .slice(0, 4),
    ],
    [
      'ENDING SOON',
      visible.filter(
        (x) =>
          daysUntil(x.deadline) === 'today' ||
          daysUntil(x.deadline) === '1 day left' ||
          daysUntil(x.deadline) === '2 days left'
      ),
    ],
    ...
    <div>
      <section className="pt-12 pb-7">
        ...
        <p className="mt-4 max-w-xl text-sm text-ink-2">
          Opportunities selected for you. Discover work posted by other
          students, then choose how you want to approach it.
        </p>
      </section>
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-3 mt-4 text-[9px] font-mono text-ink-3">
        ...
        <span className="flex items-center gap-1">
          <Clock size={11} />
          {item.deadline ? daysUntil(item.deadline) : 'No deadline'}
        </span>
      ...
      <div className="mt-4 pt-3 border-t border-metal-1/40 flex items-center gap-2">
        ...
        >
          <Heart
            size={12}
            fill={proposalSent ? 'currentColor' : 'none'}
          />
