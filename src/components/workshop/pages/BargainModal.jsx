...
import { HandCoins, X, Send, IndianRupee } from 'lucide-react';
export function BargainModal({ item, onClose }) {
      ...
      >
        ...
        {sent ? (
          <div className="text-center py-8">
            ...
            <p className="font-display text-2xl">OFFER SENT</p>
            <p className="font-mono text-[10px] text-ink-2 mt-2">
              {posterName} will see your proposal in their requests.
            </p>
            <button
              onClick={onClose}
          ...
          <>
            <div className="flex items-center gap-3 border-b border-metal-1/40 pb-4 mb-5">
              ...
              <div>
                <p className="font-technical text-[10px]">BARGAIN / NEGOTIATE</p>
                <p className="font-mono text-[9px] text-ink-3 mt-1">{item.title}</p>
              </div>
            </div>
            <div className="surface-panel rounded-xl p-4 mb-4">
              <p className="font-technical text-[8px] text-ink-3">CURRENT AMOUNT</p>
              <p className="font-display text-3xl text-amber mt-1">₹{item.amount}</p>
            </div>
